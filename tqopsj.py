...
def enter_id():
	...
	try:
		...
		try:
			...
			userid = int(input(f"{Style.RESET_ALL}Enter User ID To {Fore.RED}Hack{Style.RESET_ALL} >>> "))
...
def tries():
	...
	try:
		...
		tries = int(input(f"{Style.RESET_ALL}Number of {Fore.RED}Hack{Style.RESET_ALL} Attempts >>> {Fore.CYAN}"))
	...
	with open('Generated.txt', 'r') as f:
		...
		for line in f:
			...
			if src.status_code == 200:
				...
				tokensvalid = tokensvalid + 1

				print(f'{Fore.GREEN}Valid Token! >>> {Fore.YELLOW}' + token + f"{Style.RESET_ALL} \n{Fore.GREEN}{tokensvalid} Tokens Saved in valid.txt{Style.RESET_ALL}")
				...
				tokensinvalid = tokensinvalid + 1

				print(f'{Fore.RED}Invalid Token >>> {Fore.YELLOW}' + token + f"{Style.RESET_ALL} \n{Fore.GREEN}{tokensinvalid} Tokens Saved In invalid.txt{Style.RESET_ALL}")
